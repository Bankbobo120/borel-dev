...
DEESEEK_API_KEY = os.getenv("DEESEEK_API_KEY", "ta_clef_deepseek")
...
async def generate_pronostic(match):
    ...
    url = "https://api.deepseek.ai/v1/chat/completions"  # Remplace par le vrai endpoint Deepseek si différent
    ...
    data = {
        "model": "gpt-4o-mini",  # Adapte si Deepseek utilise un autre modèle
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data, headers=headers) as resp:
                if resp.status == 200:
                    ...
                    return f"❌ Erreur Deepseek : statut {resp.status}"
